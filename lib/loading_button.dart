...
class _LoadingButtonState extends State<LoadingButton> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    widthAnimation = Tween(begin: 1,end: _loadingSize!.width/_expandedSize!.width).animate(CurvedAnimation(parent: animationController,curve: Curves.elasticInOut))..addListener((){
      setState(() {});
    });
    heightAnimation = Tween(begin: 1,end: _loadingSize!.height/_expandedSize!.height).animate(CurvedAnimation(parent: animationController,curve: Curves.elasticInOut))..addListener((){
      setState(() {});
    });
    opacityAnimation = Tween<double>(begin: 0,end: 1).animate(CurvedAnimation(parent: animationController,curve: Curves.linear))..addListener((){
      setState(() {});
    });
    labelOpacityAnimation = Tween<double>(begin: 1,end: 0).animate(CurvedAnimation(parent: animationController,curve: Curves.fastEaseInToSlowEaseOut))..addListener((){
      setState(() {});
    });
  ...
  Widget build(BuildContext context) {
    ...
    _expandedSize = widget.expandedSize ?? const Size(200.0, 40.0);
    _loadingSize = widget.loadingSize ?? const Size(40.0, 40.0);
    _expandedSize ??= Size(Theme.of(context).buttonTheme.minWidth, Theme.of(context).buttonTheme.height);
    _loadingSize ??= Size(Theme.of(context).buttonTheme.height, Theme.of(context).buttonTheme.height);
        ...
        if(widget.state == LoadingButtonState.loading) ...[
          IgnorePointer(
            ...
            child: Opacity(
              ...
              child: CircularProgressIndicator(color: widget.type == LoadingButtonType.filled ? Theme.of(context).colorScheme.onPrimary : null)
            ),
          ))
        ]
      ],
    ...
    switch(type) {
      case LoadingButtonType.elevated:
        return ElevatedButton(
            onPressed: widget.state == LoadingButtonState.idle ? widget.onPressed : (){},
            onLongPress: widget.state == LoadingButtonState.idle ? widget.onLongPress : (){},
            onHover: widget.state == LoadingButtonState.idle ? widget.onHover : (_){},
            onFocusChange: widget.state == LoadingButtonState.idle ? widget.onFocusChange : (_){},
            style: widget.style,
